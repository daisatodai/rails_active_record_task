class ExercisesController < ApplicationController
  ...
  def exercise3  
    ...
    @address = Address.joins(:orders)
      .select('addresses.*, COUNT(orders.*) AS number_of_orders')
      .group('addresses.id').order('orders_count DESC').first
  end
  ...
  def exercise4 
    ...
    @customer = Customer.joins(orders: :foods)
      .select('customers.*, SUM(foods.price)')
      .order('foods_price_sum DESC').first
  end
end